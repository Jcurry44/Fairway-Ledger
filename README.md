...
- **Shared voice-recap inbox (optional private sync)** — a finished recap arrives as a reviewable draft with its full summary, hole narration, and coaching. It is not history until the user explicitly chooses **Apply as new round**, and at apply time the app's own course catalog overrides the draft's hole facts (par/yards/labels), so a voice draft can never corrupt analytics. Sign-in is email-based with a 6-digit code path that completes inside the installed PWA (no redirect dependence). Setup: [`supabase/README.md`](supabase/README.md) for the backend, [`docs/VOICE_SETUP.md`](docs/VOICE_SETUP.md) for the ChatGPT postgame-brief GPT and its submit action.
