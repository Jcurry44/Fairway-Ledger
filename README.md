...
- **Shared voice-recap inbox (optional private sync)** — a finished recap arrives as a reviewable draft with its full summary, hole narration, and coaching. It is not history until the user explicitly chooses **Apply as new round**. The app uses Supabase email auth + row-level security when configured; see [`supabase/README.md`](supabase/README.md) for the one-time private-service setup.
