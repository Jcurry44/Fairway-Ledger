...
(function (root, factory) {
  ...
  /**
   * Fit a rectangular region of the source image into the padded viewport.
   * The returned transform uses the same top-left origin and constraints as
   * fitView, zoomViewAt, and panView. Regions at an image edge may be shifted
   * away from the viewport center so the transform never exposes empty space.
   */
  function fitImageBoundsView(pixelBoundsInput, imageSizeInput, containerSizeInput, optionsInput) {
    if (!pixelBoundsInput || typeof pixelBoundsInput !== "object") {
      throw new TypeError("pixelBounds must be an object.");
    }
    const pixelBounds = {
      minX: finiteNumber(pixelBoundsInput.minX, "pixelBounds.minX"),
      minY: finiteNumber(pixelBoundsInput.minY, "pixelBounds.minY"),
      maxX: finiteNumber(pixelBoundsInput.maxX, "pixelBounds.maxX"),
      maxY: finiteNumber(pixelBoundsInput.maxY, "pixelBounds.maxY"),
    };
    if (pixelBounds.minX >= pixelBounds.maxX || pixelBounds.minY >= pixelBounds.maxY) {
      throw new RangeError("pixelBounds must have minX < maxX and minY < maxY.");
    }
    ...
    if (
      pixelBounds.minX < 0
      || pixelBounds.minY < 0
      || pixelBounds.maxX > image.width
      || pixelBounds.maxY > image.height
    ) {
      throw new RangeError("pixelBounds must stay within imageSize.");
    }

    const limits = viewScaleLimits(image, containerSizeInput, optionsInput);
    const boundsWidth = pixelBounds.maxX - pixelBounds.minX;
    const boundsHeight = pixelBounds.maxY - pixelBounds.minY;
    const scale = clamp(
      Math.min(limits.innerWidth / boundsWidth, limits.innerHeight / boundsHeight),
      limits.minScale,
      limits.maxScale,
    );
    const x = limits.padding.left
      + (limits.innerWidth - boundsWidth * scale) / 2
      - pixelBounds.minX * scale;
    const y = limits.padding.top
      + (limits.innerHeight - boundsHeight * scale) / 2
      - pixelBounds.minY * scale;

    return constrainView({ scale, x, y }, image, containerSizeInput, optionsInput);
  }
