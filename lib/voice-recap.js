...
(function (root, factory) {
  ...
  function normalizeDraft(input) {
    ...
    return {
      ...
      round,
      recap: { title: trimDraftText(rawRecap.title, 240), summary: trimDraftText(rawRecap.summary || rawRecap.roundSummary), coaching: Array.isArray(rawRecap.coaching) ? rawRecap.coaching.map((line) => trimDraftText(line, 4000)).filter(Boolean) : [], nextRoundCue: trimDraftText(rawRecap.nextRoundCue, 500), holeNarration, evidence: Array.isArray(rawRecap.evidence) ? copyDraftValue(rawRecap.evidence) : [] }
    };
  }
  ...
  function toAppliedRound(input, makeRound, makeId, course) {
    const result = validateDraft(input);
    if (!result.valid) throw new Error(result.errors[0]);
    const draft = result.draft;
    if (course) {
      // The app's course catalog is the authority on hole identity (number,
      // label, par, yards, hcp); the voice service is the authority on what
      // the golfer narrated. Merging here means a draft can never corrupt
      // par/yardage analytics with misremembered course facts, and the voice
      // side only has to send per-hole scores and narration.
      if (!Array.isArray(course.holes) || course.holes.length !== draft.round.holes.length) {
        fail(`This draft has ${draft.round.holes.length} scored holes but the selected route has ${Array.isArray(course.holes) ? course.holes.length : 0}.`);
      }
      draft.round.holes = course.holes.map((canonical, index) => ({
        ...draft.round.holes[index],
        number: canonical.number,
        label: canonical.label || String(canonical.number),
        par: canonical.par,
        yards: canonical.yards != null ? canonical.yards : (draft.round.holes[index].yards != null ? draft.round.holes[index].yards : null),
        hcp: canonical.hcp != null ? canonical.hcp : null
      }));
      if (course.tee) draft.round.tee = course.tee;
    }
