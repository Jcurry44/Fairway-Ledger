...
(function (root, factory) {
  ...
  function createCourseMapController(options) {
    ...
    function annotationMarkup() {
      if (!labelsEnabled) return "";
      return annotationsForCurrentHole().map((feature) => {
        const points = featurePoints(feature);
        if (!points.length) return "";
        const kind = feature.properties.kind;
        const selected = state.editor.selectedId === feature.id ? " is-selected" : "";
        const className = `course-map-annotation course-map-annotation--${escapeXml(kind)}${selected}`;
        const displayLabel = feature.properties.name || feature.properties.label;
        if (feature.geometry.type === "Point") {
          const point = points[0];
          return `<g class="${className}" data-course-map-annotation-id="${escapeXml(feature.id)}" transform="translate(${point.x.toFixed(2)} ${point.y.toFixed(2)})"><circle r="9"></circle><path d="M-13 0H13M0-13V13"></path><text x="13" y="-11">${escapeXml(displayLabel)}</text></g>`;
        }
        ...
        return `<g class="${className}" data-course-map-annotation-id="${escapeXml(feature.id)}"><path d="${d} Z"></path><text x="${center.x.toFixed(2)}" y="${center.y.toFixed(2)}">${escapeXml(displayLabel)}</text></g>`;
      }).join("");
    }
    ...
    function annotationAtPoint(point) {
      const features = annotationsForCurrentHole();
    ...
    function selectAnnotation(id) {
      ...
      const feature = annotationsForCurrentHole().find((item) => item.id === id);
    ...
    function deleteAnnotation(id) {
      ...
      if (!featureId || !annotationsForCurrentHole().some((feature) => feature.id === featureId)) return false;
      writeAnnotations(labelsApi.removeFeature(readAnnotations(), featureId));
      state.editor.selectedId = null;
      state.editor.points = [];
      if (labelElements.featureLabel) labelElements.featureLabel.value = "";
      setStatus("Draft map label deleted.", "success");
      renderLayers();
      return true;
    }
