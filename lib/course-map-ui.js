...
(function (root, factory) {
  ...
  function createCourseMapController(options) {
    ...
    function selectedHoleIdentity() {
      if (!finite(state.selectedHole)) return null;
      ...
      const supplied = typeof options.getHoleIdentity === "function"
        ...
        : null;
      const source = supplied || hole;
      const holeId = labelsApi && typeof labelsApi.holeIdentity === "function"
        ? labelsApi.holeIdentity(source)
        : (source && source.holeId);
    ...
    function currentHoleFocus() {
      const identity = selectedHoleIdentity();
      const holeView = identity && config.holeViews && config.holeViews[identity.holeId];
      const pixelBounds = holeView && holeView.pixelBounds;
      if (!pixelBounds
        || !finite(Number(pixelBounds.minX))
        || !finite(Number(pixelBounds.minY))
        || !finite(Number(pixelBounds.maxX))
        || !finite(Number(pixelBounds.maxY))) return null;
      const normalized = {
        minX: Number(pixelBounds.minX),
        minY: Number(pixelBounds.minY),
        maxX: Number(pixelBounds.maxX),
        maxY: Number(pixelBounds.maxY),
      };
      if (normalized.minX < 0
        || normalized.minY < 0
        || normalized.maxX > imageSize.width
        || normalized.maxY > imageSize.height
        || normalized.minX >= normalized.maxX
        || normalized.minY >= normalized.maxY) return null;
      return { ...identity, pixelBounds: normalized };
    }

    function syncFitButton() {
      if (!elements.fit) return;
      const focusAvailable = Boolean(currentHoleFocus()
        && typeof engine.fitImageBoundsView === "function");
      elements.fit.dataset.focusAvailable = String(focusAvailable);
      elements.fit.setAttribute("aria-pressed", String(focusAvailable && state.focusMode === "course"));
      if (!focusAvailable) {
        elements.fit.textContent = "Fit";
        elements.fit.setAttribute("aria-label", "Fit the full course in view");
        return;
      }
      const showingHole = state.focusMode === "hole";
      elements.fit.textContent = showingHole ? "Course" : "Hole";
      elements.fit.setAttribute("aria-label", showingHole
        ? "Show the full course"
        : `Focus on ${selectedHoleIdentity().holeLabel}`);
    }

    function fitWholeCourse() {
      if (!state.open) return;
      const size = viewportSize();
      state.focusMode = "course";
      setView(engine.fitView(imageSize, size, viewOptions));
      syncFitButton();
    }

    function fitSelectedHole() {
      if (!state.open) return false;
      const focus = currentHoleFocus();
      if (!focus || typeof engine.fitImageBoundsView !== "function") {
        fitWholeCourse();
        return false;
      }
      state.focusMode = "hole";
      setView(engine.fitImageBoundsView(focus.pixelBounds, imageSize, viewportSize(), viewOptions));
      syncFitButton();
      return true;
    }

    function fit() {
      return fitSelectedHole();
    }

    function toggleFitView() {
      const focusAvailable = Boolean(currentHoleFocus()
        && typeof engine.fitImageBoundsView === "function");
      if (!focusAvailable || state.focusMode === "hole") {
        fitWholeCourse();
        return;
      }
      fitSelectedHole();
    ...
    function currentHoleIdentity() {
      return labelsEnabled ? selectedHoleIdentity() : null;
    }
    ...
    function open(input) {
      ...
      requestAnimationFrame(fitSelectedHole);
    ...
    elements.fit.addEventListener("click", toggleFitView);
